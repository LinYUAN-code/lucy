...
`)});function A(d,s,c,v){var _=c?"":v.css;if(d.styleSheet)d.styleSheet.cssText=R(s,_);else{var m=document.createTextNode(_),L=d.childNodes;L[s]&&d.removeChild(L[s]),L.length?d.insertBefore(m,L[s]):d.appendChild(m)}}},function(k,x){k.exports=function(f){var o=typeof window<"u"&&window.location;if(!o)throw new Error("fixUrls requires window.location");if(!f||typeof f!="string")return f;var l=o.protocol+"//"+o.host,u=l+o.pathname.replace(/\/[^\/]*$/,"/");return f.replace(/url\s*\(((?:[^)(]|\((?:[^)(]+|\([^)(]*\))*\))*)\)/gi,function(g,b){var a,e=b.trim().replace(/^"(.*)"$/,function(t,n){return n}).replace(/^'(.*)'$/,function(t,n){return n});return/^(#|data:|http:\/\/|https:\/\/|file:\/\/\/|\s*$)/i.test(e)?g:(a=e.indexOf("//")===0?e:e.indexOf("/")===0?l+e:u+e.replace(/^\.\//,""),"url("+JSON.stringify(a)+")")})}}]).default})})(ce);(function(E){E.exports=V})(le);const ue=G(P);const z=E=>(se("data-v-f3330aa0"),E=E(),ae(),E),pe={class:"analysis"},fe={class:"content"},he={class:"input-string"},be={class:"first"},ve={class:"parser-string"},ye=z(()=>U("h3",null,"Ast Explore",-1)),me=z(()=>U("div",{id:"astNodeContainer"},null,-1)),ge={__name:"LL1Analysis",setup(E){X();const B=K();Q({});const k=T(()=>B.getters["grammarStore/getStartTNonTer"]),x=T(()=>B.getters["grammarStore/getLL1ParserString"]),f=T(()=>B.getters["grammarStore/getLL1Parser"]),o=T(()=>{const b=B.getters["grammarStore/getFirstSet"],a=B.getters["grammarStore/getFollowSet"];return f.value.getPredictTable(b,a)});T(()=>[...B.getters["grammarStore/getTerminal"],"$"]),T(()=>o.value.length?o.value.map(a=>{const{nonTerminal:e="",terminal2Derivation:t={}}=a,n=new Map;return t.forEach((r,i)=>{const{derivations:p=[],nonTerminal:h=""}=r,w=p.map(N=>N.length?`${h} => ${N.join(" ")}`:"");n.set(i,w)}),{nonTerminal:e,...Object.fromEntries(n.entries())}}):[]);const l=$([]),u=()=>{if(!x.value||!k.value)return[];let b;try{b=f.value.getPredictProcess(x.value,k.value,o.value),ie(()=>{console.log("ast",b.astNode);const e=new ue("#astNodeContainer",{data:[b.astNode]})})}catch(e){console.error(e)}const a=b.map((e,t)=>{var n,r,i;return{Step:t+1,Stack:(i=(r=(n=e.parseStack)==null?void 0:n.slice())==null?void 0:r.reverse())==null?void 0:i.join(""),Input:e.remainingInput+"$",Action:e.parseAction}});l.value=a},g=$(!1);return Y([()=>x.value,k],([b,a],[e,t])=>{!b||!a?g.value=!0:u()},{immediate:!0,deep:!0}),(b,a)=>{const e=F("el-table-column"),t=F("el-table");return Z(),ee("div",pe,[O(de),U("div",fe,[U("div",he,[U("div",be,[te(" LL(1)预测分析 "),U("span",ve,ne(re(x)),1)])]),O(t,{data:l.value,stripe:"",style:{width:"100%"},border:"",class:"table"},{default:oe(()=>[O(e,{prop:"Step",label:"Step",align:"center"}),O(e,{prop:"Stack",label:"Stack",align:"center"}),O(e,{prop:"Input",label:"Input",align:"center"}),O(e,{prop:"Action",label:"Action",align:"center"})]),_:1},8,["data"])]),ye,me])}}},xe=W(ge,[["__scopeId","data-v-f3330aa0"]]);export{xe as default};
